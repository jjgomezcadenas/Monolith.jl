...
function draw_rcube(mcr::Rcub, pdf=nothing, rdf=nothing, plotg = false, np=100)
	dm = dimensions(mcr)
...
function sicrys(dx::Float64,dy::Float64,dz::Float64, ng=10; plot=true)
...
sicrys(50.0,50.0,18.6*2,1000, plot=true)
...
ixdf = crgun(50.0,50.0,18.6*2,ng=50000, xsipm=3)
...
# ╔═╡ 2ce08034-3bc2-4503-835d-163bf84529a4
Gray.(imgxy)
...
Input:
 ...
 - seed can be controled. 
 - returns an image of the Z detection plane as seen by SiPMs
"""
function ggun(dm::NamedTuple; ng::Integer=1000, xsipm::Float64=3.0, pde::Float64=1.0,
              rpos=true, xg::Float64=0.0, yg::Float64=0.0, zg::Float64=0.0,
              seed=12345, eps=1e-5)
	...
	#vdf =vectors_spherical2(ng) # direction vectors
	vdf =vectors_spherical2(ng, eps, seed)
	...
	DataFrame(eg=ng, xg=x, yg=y, zg=z), exy ./ maximum(dfbx.ebox)  #gray scale, normalize to one
end

# ╔═╡ f02465eb-e85c-4603-bd7f-ecee57afe77f
lblx, imgx = ggun(dm, ng=50000, xsipm=3.0, pde=1.0, rpos=true)
...
lbly, imgy = ggun(dm, ng=50000, xsipm=3.0, pde=1.0, rpos=false, xg=22.5, yg=35.1, zg=28.0)
...
function imagerun(img0, imgl; dm, ng=50000, xsipm=3.0, pde=1.0, rpos=true)
	rflbl = "lbl"
	rfimg = "img"
	for i in img0:imgl
...
imagerun(1, 100; dm, ng=50000, xsipm=3.0, pde=1.0, rpos=true)

# ╔═╡ 3749a077-626b-433c-82b5-06e4d2a016bb
function ncr(dpet, cl)
	L = 2π*dpet/2
	floor(L/cl)
end

# ╔═╡ 1fa7dd29-80d0-4eca-a920-cb6c251b284c
function ncl(lpet, cl)
	floor(lpet/cl)
end

# ╔═╡ cec725a9-c6f6-488f-a810-84f8778a5ea6
ncr(1e+3,50)

# ╔═╡ 9fb53d4f-48c4-4b02-84be-91cb551a4834
ncl(1e+3,50)

# ╔═╡ 86c9e88c-841e-4435-bf88-29de366ad88a
ncr(1e+3,50) * ncl(1e+3,50)

# ╔═╡ 0df9ffb1-6eec-42e2-b4f6-ea3b63498e43
cst(nc, cost=530) = nc * cost

# ╔═╡ 04f172ad-0387-422e-8fac-75fc5b668021
cst(ncr(1e+3,50)*ncl(1e+3,50))

# ╔═╡ 17e0f6e5-f456-42be-804d-4fdd2c6e9664
ncr(100,50)

# ╔═╡ 4e00bbd5-f2fa-49c7-8388-bf335a0832f8
ncl(100,50)

# ╔═╡ 8f0a4df2-e486-4732-b5a6-ac900f887951
cst(ncr(1e+2,50)*ncl(1e+2,50))

# ╔═╡ 25957ced-ba22-48c1-93ce-3bb0940ccccc
1200 * 64
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "0c3f1df2e535a27583a4a3f1ee8f28c898059d10"
